...
class ScmConnectorConfiguration {

    @Bean
    @ConditionalOnMissingBean(ScmConnectorFactory::class)
    fun connectorFactory(
        @Autowired(required = false) metricsEventListener: OkHttpMetricsEventListener?
    ) = OkHttpScmConnectorFactory(metricsEventListener)

    @Bean
    @ConditionalOnClass(MeterRegistry::class)
    @ConditionalOnProperty(prefix = "management.metrics", name = ["enabled"], matchIfMissing = true)
    fun okHttpMetricsEventListener(meterRegistry: MeterRegistry): OkHttpMetricsEventListener {
